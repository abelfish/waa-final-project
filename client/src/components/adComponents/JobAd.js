...
function JobAd(props) {
  const imageUrl = function (image) {
    return image ? image : 'https://picsum.photos/200/300';
  };
  ...
  return (
    <div key={props.id}>
      <div
        key={props.id}
        className="m-auto overflow-hidden rounded-lg shadow-lg cursor-pointer w-60 md:w-80"
      >
        <Link to={'/job-ad/' + props.id} className="block w-full h-full">
          <img
            alt="Job Ad"
